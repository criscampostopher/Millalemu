/* ==========================================================
  Lógica del mapa, GPS y alertas (CORREGIDO)
   ========================================================== */
...
window.reportarUser = function() {
    ...
    navigator.geolocation.getCurrentPosition(p => {
        ...
        saveMarker({lat: p.coords.latitude, lng: p.coords.longitude}, "🚨 SOS - OPERADOR", msg, "Critico", 20, idDestino, "✅ SOS Enviado con éxito");
    }, err => {
        document.getElementById('loader').style.display = 'none';
...
function saveMarker(ll, nom, desc, nivel, radio = 0, id_destino = 0, mensajePersonalizado = "✅ Acción realizada") {
    ...
    .then(res=>{
        document.getElementById('loader').style.display = 'none';
        if(res.success) { alert(mensajePersonalizado); location.reload(); }
...
// --- 7. INICIALIZAR MAPA (FUNCIÓN PRINCIPAL) ---
function initMap() {
    // Definir mapa y vista inicial
        ...
        if(IS_ADMIN) {
            ...
            if(sel) {
                ...
                LISTA_MAPAS.forEach(m => { 
                    let opt = document.createElement('option'); opt.value = m.id_mapa; opt.text = "📍 " + m.nombre_mapa; 
                ...
                res.markers.forEach(row => {
                    try {
                        ...
                        if (geom.type === 'Point') {
                            ...
                            if (m.radio_custom > 0) { L.circle([geom.coordinates[1], geom.coordinates[0]], { radius: m.radio_custom, color: '#e74c3c', fillColor: '#c0392b', fillOpacity: 0.3, weight: 1 }).addTo(layerManuales); }
                        }
                    } catch(e){}
                });
                if(ultimaPosicion) checkPeligros(ultimaPosicion[0], ultimaPosicion[1]);
    ...
    document.getElementById('markerForm').onsubmit = (e) => {
        ...
        saveMarker(currentLatLng, document.getElementById('popupNombre').value, document.getElementById('popupDesc').value, document.getElementById('popupIcon').value, radioVal ? parseInt(radioVal) : 0, 0, "✅ Reporte creado correctamente");
        form.style.display='none';
    };
    
    const btnBorrar = document.getElementById('borrarMarcadores');
    if(btnBorrar) {
        btnBorrar.onclick = () => { if(confirm("⚠️ ¿RESETEAR TODO EL SISTEMA?")) fetch('Api/api_mapa.php?action=delete_all', { method: 'POST' }).then(() => location.reload()); };
    }
